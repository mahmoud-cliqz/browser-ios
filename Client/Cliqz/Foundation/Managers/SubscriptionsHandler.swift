...
//        }
    ...
    private func showSubscribeAlert() {
        ...
        if case let .subscriptoinNotification(_, subType, _) = subscriptionType {
            ...
            let parameterizedTitle = NSLocalizedString("Would you like to subscribe to the {1}?", tableName: "Cliqz", comment: "[PushNotification] Alert title for asking to subscribe for soccer notificaiton")
            let parameterizedMessage = NSLocalizedString("You will be notified about games, goals and important happenings of the {1}.", tableName: "Cliqz", comment: "[PushNotification] Alert message for asking to subscribe for soccer notificaiton")
            let localizedSubType = NSLocalizedString(subType, tableName: "Cliqz", comment: "[PushNotification] soccer notification")
            ...
            let subscribeAction = UIAlertAction(title: NSLocalizedString("Subscribe", tableName: "Cliqz", comment: "[PushNotification] Subscribe alert button"), style: .default, handler: {[weak self] (_) in
    ...
    private func showEnableNotificationFromSystemAlert() {
        ...
        if case let .subscriptoinNotification(_, subType, _) = subscriptionType {
            ...
            let parameterizedTitle = NSLocalizedString("Could not subscribe to {1}?", tableName: "Cliqz", comment: "[PushNotification] Alert title for asking to enable push notificaiton from the system")
            let localizedSubType = NSLocalizedString(subType, tableName: "Cliqz", comment: "[PushNotification] soccer notification")
            
            let alertTitle = parameterizedTitle.replace("{1}", replacement: localizedSubType)
            
            let alertMessage = NSLocalizedString("Activate push notification to use this feature. Go to settings to allow push notifications then retry.", tableName: "Cliqz", comment: "[PushNotification] Alert message  for asking to enable push notificaiton from the system")
            
            let cancelAction = UIAlertAction(title: NSLocalizedString("Cancel", tableName: "Cliqz", comment: "Cancel"), style: .default, handler: {(_) in
                TelemetryLogger.sharedInstance.logEvent(.Subscription("click", "permission_alert", ["target" : "cancel"]))
            })
            
            let settingsAction = UIAlertAction(title: NSLocalizedString("Settings", tableName: "Cliqz", comment: "[PushNotification] Subscribe alert button"), style: .default, handler: {(_) in
    ...
    private func showFailedToSubscribeAlert() {
        ...
        if case let .subscriptoinNotification(_, subType, _) = subscriptionType {
            ...
            let parameterizedTitle = NSLocalizedString("Could not subscribe to {1}?", tableName: "Cliqz", comment: "[PushNotification] Alert title when failing to subscribe to soccer notification")
            let localizedSubType = NSLocalizedString(subType, tableName: "Cliqz", comment: "[PushNotification] soccer notification")
            
            let alertTitle = parameterizedTitle.replace("{1}", replacement: localizedSubType)
            
            let alertMessage = NSLocalizedString("Sorry, we encountered some technical difficulties. Please try again", tableName: "Cliqz", comment: "[PushNotification] Alert message when failing to subscribe to soccer notification")
            ...
            let settingsAction = UIAlertAction(title: NSLocalizedString("Retry", tableName: "Cliqz", comment: "[PushNotification] Retry alert button"), style: .default, handler: {[weak self] (_) in
                self?.registerForUserNotifications()
    ...
    private func showSubscribeHUD() {
        ...
        if case let .subscriptoinNotification(_, subType, _) = subscriptionType {
            ...
            let parameterizedMessage = NSLocalizedString("Subscribe to {1}?", tableName: "Cliqz", comment: "[PushNotification] Subscribing HUD")
            let localizedSubType = NSLocalizedString(subType, tableName: "Cliqz", comment: "[PushNotification] soccer notification")
